...
dataset = datasets.fetch_openml("mnist_784", as_frame=False)
data = dataset.data
...
# otherwise, we are using "channels last" ordering, so the design matrix shape should be: num_samples x rows x columns x depth
else:
